...
> **STATUS 2026-08-11: mostly carried out.** Everything in §1 Tier 3 is deleted, the docs are
> consolidated here in `docs/`, the configs have a README, and §3's four reproducibility gaps are
> now stated in the top-level README instead of being left to discovery.
>
> **Three items are deliberately NOT done, and each is blocked on something real:**
>
> | not done | blocked on |
> |---|---|
> | §2(a)(b) moving `src/maskformer/hepattn_colliderml/` and the launchers to a top-level `model/` | **a live training run.** `ce_ai_1/env.sh` copies that directory into the hepattn checkout at launch, and a resume re-reads it. Moving it now breaks `CKPT=... ./train_pu0.sh`. Do it when no run is in flight — it is a pure `git mv` plus paths in `README.md`, `env.sh`, `verify_sync.sh`, `paths.sh` and `install_training_env.sh`. |
> | replacing `results/pu0/` wholesale | **the new run has not finished.** These are the only pu0 numbers that exist. The duplicates (`_v1`, `_v2`, `_smoke`) and the logs are gone; what remains is marked by `results/pu0/STALE.md` and `figures/pu0/STALE.md`. |
> | deleting `src/maskformer/dias/` | **the same run.** §1 Tier 4 conditions this on the new pu0 result landing on ce-ai-1. Until it does, DIAS is still the provenance for the only pu0 numbers in the tree. |
>
> Two Tier 4 items were resolved *against* deleting, on evidence rather than taste:
> `soft.py` / `score_soft.py` / `scan_soft_threshold.py` stay because the multi-owner capability
> study is discussed in `METHODOLOGY_PLAN.md` and `DISCUSSION_PLAN.md`; `compare_probes.py` stays
> because the probe analysis is in `DISCUSSION_PLAN.md`.
>
> **One Tier 4 item is left for you to decide, because the evidence points at deleting and it is
> your recent work.** `scripts/analyse_anatomy.py` and its fourteen `figures/pu200/anatomy_*`
> outputs are **not cited anywhere** — `RESULTS_PLAN.md` §4.4 covers "the anatomy of the
> difference" with `fig_cluster_size` and `fig_shower_profile`, which `make_thesis_figures.py`
> produces from the same `src/evaluation/anatomy.py` library. So the library stays either way;
> the question is only whether the exploratory seven-figure script earns its place. Kept for now.
