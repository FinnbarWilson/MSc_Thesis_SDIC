#!/bin/bash
# Check that the copies in this directory still match the hepattn working tree they came from.
#
# These files are duplicated rather than imported, which buys the dependency boundary described
...
if git -C "$HEPATTN" diff --quiet src/hepattn/models/loss.py src/hepattn/models/task.py \
  ...
  status=1
     ...
     | diff -q - "$HERE/hepattn-changes.patch" >/dev/null 2>&1; then
...
if [ "$status" -eq 0 ]; then
  ...
  echo "      src/hepattn/callbacks/prediction_writer.py > $HERE/hepattn-changes.patch"
fi
exit "$status"
