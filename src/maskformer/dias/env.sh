...
#
# The counterpart to ce_ai_1/env.sh: DIAS has a scheduler, needs a container, and has one faulty
# GPU to route around.
#
# The repository root. Slurm copies a submitted script into a spool directory, so
# ${BASH_SOURCE[0]} is useless under sbatch; SLURM_SUBMIT_DIR is the directory sbatch was run
# from, which is the repository root if you submit from there. REPO= overrides both.
...
# bare base image and ships no cc/gcc/g++ at all. Triton does not just want a compiler for
# torch.compile: it shells out to one during driver initialisation, to build its CUDA utils
# module, so any Triton kernel launch needs it. A run died this way ~4 minutes in with
...
# configs/ is whatever was last copied there, so a relative --config can silently train a stale copy.
DATASET="${DATASET:-pu0}"
...
# do the same.
#
# Measured at 24 configured workers: 48 worker processes (train and val dataloaders both
# live) holding 960 threads on 32 permitted cores, a 30x oversubscription. The result after 35
...
# three cards whatever was allocated, verified in a job that asked for two and saw indices
...
# error" minutes in. Requesting two cards guarantees the allocation contains a healthy one: the
# node has three and only one is faulty. Only one is ever used; trainer.devices stays 1.
#
# Its uncorrected count is still climbing while the other two sit at zero. RE-CHECK THAT before
# trusting this: if a second card goes bad, two no longer guarantees a good one.
select_gpu() {
    ...
    for idx in ${alloc//,/ }; do
        [ -n "$first" ] || first="$idx"
        # Both counters, not just volatile: the volatile count resets on a driver reload while
        # the aggregate one lives in the inforom and survives. The faulty card has been seen
        # reading volatile=0 against a four-figure aggregate, which a volatile-only check would
        # call healthy. A card is a candidate only if both are zero.
...
#
# This couples the run to --mem. Slurm here applies VSizeFactor, setting `ulimit -v` to 1.1 x
# --mem, and expandable_segments reserves a large virtual address range up front, so under a small
# cap CUDA context creation itself fails, as "RuntimeError: CUDA driver error: out of memory" on
# a card with 78 GB free and no processes on it, which reads like a GPU problem and is not one.
#
